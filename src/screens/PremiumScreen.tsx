...
const PremiumScreen: React.FC<Props> = ({ appState }) => {
  ...
  return (
    <div style={{ background: '#0D0D0F', minHeight: '100vh' }}>
      {/* Header */}
      <div style={{
        background: 'linear-gradient(180deg, rgba(123,47,190,0.35) 0%, transparent 70%)',
        padding: '50px 20px 24px', textAlign: 'center',
      }}>
        <button
          style={{ position: 'absolute', top: 50, left: 20, background: 'none', border: 'none', color: 'rgba(255,255,255,0.6)', fontSize: 22, cursor: 'pointer' }}
        ...
        <div style={{ fontSize: 28, fontWeight: 900, marginBottom: 8 }}>
          <span style={{ color: '#FFD700', textShadow: '0 0 20px rgba(255,215,0,0.5)' }}>BMU</span> Premium
        </div>
        <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.65)', lineHeight: 1.6, maxWidth: 280, margin: '0 auto' }}>
          Unlock your full potential with the complete BMU experience
        </div>
      </div>
      ...
      <div style={{ padding: '0 20px 40px' }}>
        ...
        <div style={{
          ...
          {[
            ...
            { id: 'yearly', label: 'Yearly  🎉 Save 40%' },
          ].map(p => (
            <button
              key={p.id}
              ...
              style={{
                ...
                background: plan === p.id ? 'linear-gradient(135deg, #7B2FBE, #FF2D78)' : 'transparent',
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'rgba(255,255,255,0.5)', marginBottom: 14, letterSpacing: 0.5 }}>
        ...
        <div className="card" style={{ padding: 20, marginBottom: 24 }}>
          <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>Free vs Premium</div>
          {freeFeatures.map((f, i) => (
            <div key={i} style={{
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              padding: '8px 0', borderBottom: i < freeFeatures.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none',
            }}>
              <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)' }}>{f}</span>
              <div style={{ display: 'flex', gap: 20 }}>
                <span style={{ fontSize: 14, color: '#4CAF50' }}>✓</span>
                <span style={{ fontSize: 14, color: '#9B59B6' }}>✓</span>
              </div>
            </div>
          ))}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 20, paddingTop: 12 }}>
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', fontWeight: 600 }}>FREE</span>
            <span style={{ fontSize: 11, color: '#9B59B6', fontWeight: 600 }}>PREMIUM</span>
          </div>
        </div>
        ...
        <div style={{ textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,0.3)', lineHeight: 1.8 }}>
          Cancel anytime. No hidden fees.<br />
          Your subscription helps fund free access for those in need. 💙
        </div>
      </div>
    </div>
  );
